...
describe("Secp256k1", () => {
	test("Can generate a key pair from a seed", () => {
		...
		const keyPair = Secp256k1.keyPairFromPrivateKey(seed);
	...
	test("Can generate a signature with a private key", () => {
		...
		const sig = Secp256k1.sign(keyPair.privateKey, data);

		expect(Converter.bytesToHex(sig)).toEqual(
			"3045022100bf1a01d3e1d70687343bfaea90f678a40c7549db9a9f296fdb16c577486592e60220674681ec9d1742d9716ced405cab6e53406f6ba8daafae18d76282dd4392f317"
		);
	});
	...
	test("Can validate a signature with the public key", () => {
		...
		const verified = Secp256k1.verify(keyPair.publicKey, data, sig);
