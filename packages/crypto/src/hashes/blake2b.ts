...
export class Blake2b {
	 ...
	 * @internal
	 */
	private static readonly _BLAKE2B_IV32 = new Uint32Array([
	 ...
	 * @internal
	 */
	private static readonly _SIGMA8 = [
	 ...
	 * @param outputLength Output length between 1 and 64 bytes.
	 * @param key Optional key.
	 * @throws Error if the output length is not between 1 and 64 bytes.
	 */
	private init(outputLength: number, key?: Uint8Array): void {
		if (outputLength <= 0 || outputLength > 64) {
			throw new GeneralError(Blake2b._CLASS_NAME, "outputLength64", { outputLength });
		}
		if (key && key.length > 64) {
			throw new GeneralError(Blake2b._CLASS_NAME, "keyLength64", { keyLength: key.length });
		}

		// initialize hash state
		for (let i = 0; i < 16; i++) {
			this._context.h[i] = Blake2b._BLAKE2B_IV32[i];
		}
		const keylen = key ? key.length : 0;
		this._context.h[0] ^= 0x01010000 ^ (keylen << 8) ^ outputLength;
	...
	private compress(last: boolean): void {
		...
		for (i = 0; i < 16; i++) {
			...
			this._v[i + 16] = Blake2b._BLAKE2B_IV32[i];
		}
		...
		for (i = 0; i < 12; i++) {
			this.b2bG(0, 8, 16, 24, Blake2b._SIGMA82[i * 16 + 0], Blake2b._SIGMA82[i * 16 + 1]);
			this.b2bG(2, 10, 18, 26, Blake2b._SIGMA82[i * 16 + 2], Blake2b._SIGMA82[i * 16 + 3]);
			this.b2bG(4, 12, 20, 28, Blake2b._SIGMA82[i * 16 + 4], Blake2b._SIGMA82[i * 16 + 5]);
			this.b2bG(6, 14, 22, 30, Blake2b._SIGMA82[i * 16 + 6], Blake2b._SIGMA82[i * 16 + 7]);
			this.b2bG(0, 10, 20, 30, Blake2b._SIGMA82[i * 16 + 8], Blake2b._SIGMA82[i * 16 + 9]);
			this.b2bG(2, 12, 22, 24, Blake2b._SIGMA82[i * 16 + 10], Blake2b._SIGMA82[i * 16 + 11]);
			this.b2bG(4, 14, 16, 26, Blake2b._SIGMA82[i * 16 + 12], Blake2b._SIGMA82[i * 16 + 13]);
			this.b2bG(6, 8, 18, 28, Blake2b._SIGMA82[i * 16 + 14], Blake2b._SIGMA82[i * 16 + 15]);
		}
