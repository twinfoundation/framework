...
export class FieldElement {
	 ...
	 * Field element size.
	 */
	private static readonly _FIELD_ELEMENT_SIZE: number = 10;
	...
	constructor(values?: Int32Array | number[]) {
		this.data = new Int32Array(FieldElement._FIELD_ELEMENT_SIZE);
		if (values) {
	 ...
	 * 10 of them are 2-way parallelisable and vectorisable.
	...
	public combine(
        ...
        */
		...
		c0 = (h0 + BIG_1_SHIFT_LEFT_25) >> BIG_ARR[26];
		h1 += c0;
		h0 -= c0 << BIG_ARR[26];
		c4 = (h4 + BIG_1_SHIFT_LEFT_25) >> BIG_ARR[26];
		h5 += c4;
		h4 -= c4 << BIG_ARR[26];
		/* |h0| <= 2^25 */
		/* |h4| <= 2^25 */
		...
		const c1 = (h1 + BIG_1_SHIFT_LEFT_24) >> BIG_ARR[25];
		h2 += c1;
		h1 -= c1 << BIG_ARR[25];
		const c5 = (h5 + BIG_1_SHIFT_LEFT_24) >> BIG_ARR[25];
		h6 += c5;
		h5 -= c5 << BIG_ARR[25];
		...
		const c2 = (h2 + BIG_1_SHIFT_LEFT_25) >> BIG_ARR[26];
		h3 += c2;
		h2 -= c2 << BIG_ARR[26];
		const c6 = (h6 + BIG_1_SHIFT_LEFT_25) >> BIG_ARR[26];
		h7 += c6;
		h6 -= c6 << BIG_ARR[26];
		...
		const c3 = (h3 + BIG_1_SHIFT_LEFT_24) >> BIG_ARR[25];
		h4 += c3;
		h3 -= c3 << BIG_ARR[25];
		const c7 = (h7 + BIG_1_SHIFT_LEFT_24) >> BIG_ARR[25];
		h8 += c7;
		h7 -= c7 << BIG_ARR[25];
		...
		h5 += c4;
		h4 -= c4 << BIG_ARR[26];
		const c8 = (h8 + BIG_1_SHIFT_LEFT_25) >> BIG_ARR[26];
		h9 += c8;
		h8 -= c8 << BIG_ARR[26];
		...
		const c9 = (h9 + BIG_1_SHIFT_LEFT_24) >> BIG_ARR[25];
		h0 += c9 * BIG_ARR[19];
		h9 -= c9 << BIG_ARR[25];
	...
	public toBytes(bytes: Uint8Array): void {
		const carry = new Int32Array(FieldElement._FIELD_ELEMENT_SIZE);
	 ...
	 */
	public neg(): void {
		for (let i = 0; i < FieldElement._FIELD_ELEMENT_SIZE; i++) {
